...
import { Mail, Eye, EyeOff, User, Lock, AlertCircle } from "lucide-react";
import "../index.css";
...
const LoginPage = (log) => {
  ...
  const handleForgotPassword = async (e) => {
    ...
    try {
      ...
      await fetchSignInMethodsForEmail(getAuth(), email);
      const getCookie = Cookies.get("sessionToken");
      const findemail = await axios.get(
        `${
          process.env.REACT_APP_BACKEND_URL
        }findmail?email=${encodeURIComponent(email)}`,
        {
          headers: {
    ...
    try {
      ...
      if (user && !user.emailVerified) {
        ...
        try {
          ...
          const val = { email1: email, auth: auth1, token1: "" };
        ...
        Cookies.set("sessionToken", dat.token, {
          expires,
          secure: true,
          sameSite: "Strict",
        });
        ...
        const findemail = await axios.get(
          `${
            process.env.REACT_APP_BACKEND_URL
          }findemail?email=${encodeURIComponent(email)}`,
          {
            headers: {
              Authorization: `Bearer ${getCookie}`,
              "Content-Type": "application/json",
      ...
      toast.dismiss();
      toast.error(
        "Invalid email or password. If You are a New User Please Signup"
      );
  ...
  const signInWithGoogle = async () => {
    ...
    try {
      ...
      try {
        const key = CryptoJS.enc.Utf8.parse(ps.padEnd(32, " "));
        const iv = CryptoJS.enc.Utf8.parse(ps.padEnd(16, " "));
        const val = { email1: user.email, auth: auth1, token1: "" };
        const valString = JSON.stringify(val);
        encrypted = CryptoJS.AES.encrypt(valString, key, {
          iv: iv,
          mode: CryptoJS.mode.CBC,
      ...
      try {
        ...
        const findemail = await axios.get(
          `${
            process.env.REACT_APP_BACKEND_URL
          }findemail?email=${encodeURIComponent(user.email)}`,
          {
            headers: {
              Authorization: `Bearer ${getCookie}`,
              "Content-Type": "application/json",
      ...
      if (error.code === "auth/popup-blocked") {
        toast.error(
          <div>
            ...
            <p>
              Please enable pop-ups for this site to continue with Google
              Sign-In.
            </p>
            <p>
              In Safari: Safari → Settings → Websites → Pop-up Windows → Allow
            </p>
          </div>,
          {
            position: "top-center",
        ...
        toast.error(
          <div>
            <p>Browser security policy blocked the sign-in.</p>
            <p>Try refreshing the page or using a different browser.</p>
            <p>If the issue persists, try disabling browser extensions.</p>
          </div>,
          {
            position: "top-center",
            autoClose: 7000,
  ...
  const validateForm = () => {
    ...
    if (!phone && !isLogin) {
      ...
      newErrors.phone =
        "Please enter a valid mobile number in the format 1234567890";
    }
          ...
          <div className="text-center mb-3 ">
            <h1
              ...
              style={{
                ...
                whiteSpace: "nowrap",
                animation:
                  "typing 1s steps(30) 1s 1 normal both, no-blink 0s 1.4s forwards",
              }}
            >
              ...
              <div className="space-y-6">
                ...
                <>
                  ...
                  {shouldHaveRainbowEffect ? (
                    ...
                    >
                      <div
                        className="py-2 border border-gray-300 rounded-lg bg-gray-50 transition-colors flex items-center justify-center gap-2 "
                        style={{
                          width: "97.3%",
                          height: "50%",
                          marginTop: "5px",
                ...
                <>
                  ...
                  {shouldHaveRainbowEffect ? (
                    <div className="rainbow-border">
                      ...
                      >
                        <div
                          className="text-purple-600 hover:text-purple-700  transition-all duration-600 ease-in-out font-bold"
                          style={{
                            background: "white",
                            borderRadius: "2px",
                            padding: "4px",
