...
const FDRecommendationPage = ({ mail }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(
        `${process.env.REACT_APP_BACKEND_URL}recommendations/fd`,
        formData,
        {
          headers: {
  ...
  return (
    <>
      <motion.div
        className="fixed top-0 left-0 right-0 h-1 z-50 bg-blue-500/30"
        style={{ scaleX }}
      />
      ...
      {scrollYProgress > 0.2 && (
        <motion.button
          ...
          whileTap={{ scale: 0.9 }}
          className="fixed bottom-6 right-6 bg-blue-500/80 text-white p-3 rounded-full shadow-2xl z-50 hover:bg-blue-600 transition-colors"
        >
          <ArrowUp />
        </motion.button>
      )}
              ...
              {[
                {
                  name: "age",
                  label: "Age",
                  icon: <CalendarDays className="text-white/70" />,
                },
                {
                  name: "amount",
                ...
                >
                  ...
                  <input
                    ...
                    className="w-full pl-12 pr-4 py-3 bg-white/10 border border-white/30 rounded-xl text-white placeholder-white/50 focus:outline-none focus:border-white/50 focus:ring-2 focus:ring-blue-500/50 transition-all duration-300"
                    required
                  />
                </motion.div>
              ))}
            ...
            >
              ...
              <motion.p
                ...
                dangerouslySetInnerHTML={{
                  __html: formatChatbotResponse(
                    recommendation.groqRecommendation
                  ),
                }}
              ></motion.p>
            </motion.div>
...
export default FDRecommendationPage;
